...
# file_create_date: 2020/5/27 20:24
import pandas as pd
from datetime import datetime, timedelta
length = 1000
src_data = [[i for i in range(12)]] * length
df = pd.DataFrame(src_data)# 从粘贴板上读取数据
t = datetime.now().date() - timedelta(days=1)
writer = pd.ExcelWriter('样式%d%02d%02d.xlsx' % (t.year, t.month, t.day), engine='xlsxwriter')
