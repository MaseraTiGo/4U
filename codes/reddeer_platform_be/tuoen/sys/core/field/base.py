    ...
    def _check_choices(self, value):
        if self._choice_mapping:
            if value not in self._choice_mapping:
                print('parameter is not in choices')
                raise NotImplementedError('parameter is not in choices')
        return value
        ...
        if type(value) is dict:
            ...
            for key, helper in fields.items():
                ...
                try:
                    ...
                    raise Exception("parameter '{}' format error".format(key))
                else:
        ...
        for key, helper in fields.items():
            if key not in value:
                if self._is_required:
                    raise Exception("parameter '{}' losted".format(key))
                else:
                    continue
            ...
            try:
                ...
                raise Exception("parameter '{}' format error, e = {}".format(key, e))
            else:
                result[key] = cur_value
        return DictWrapper(result)
...
NewDictField = type('NewDictField', (BaseField,),
                    {'parsing': lambda _, value: value, 'formatting': lambda _, value: value})
