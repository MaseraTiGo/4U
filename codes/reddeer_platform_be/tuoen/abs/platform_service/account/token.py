...
# class Token(object):
#     _key_fmt = "{account_id}:token"
#     _token_fmt = "{account_id}{timestamp}{random_num}{role}"
...
#             result = self.calculate(self.account_id, self.role)
...
#
#         # one level cache
#         redis.set(self.auth_token, token_str, self._cache_category, ex=60 * 60 * 24)
#
#         # two level cache
#         # redis.set(self.auth_token, token_str, self._cache_category)
#
...
#         return cls._key_fmt.format(account_id=self.account_id)
#
#     @classmethod
#     def _get_time_scope(cls, hour=24):
#         cur_time = int(time.time())
#         expire_time = cur_time + hour * 60 * 60 * 2
#         return cur_time, expire_time
#
#     @classmethod
#     def calculate(cls, account_id, role):
#         cur_time, expire_time = cls._get_time_scope()
#         random_num = random.randint(1, 99999)
#         token_str = cls._token_fmt.format(account_id=account_id, \
#                                           timestamp=cur_time, random_num=random_num, role=role)
...
#
#     @classmethod
#     def generate(cls, account_id, role):
#         calc_result = cls.calculate(account_id, role)
...
#         redis.delete(auth_token, cls._cache_category)
#
#     def __str__(self):
#         return json.dumps({
#             "renew_flag": self.renew_flag,
