...
class AliConfig(Config):
    ...
    def config_details_mapping(self):
        return {
            (self.classify, 'access_key_id', '短信key',): {},
            (self.classify, 'access_secret', '短信secret'): {},
            (self.classify, 'sign_name', '签名',): {},
            (self.classify, 'agent_sign_name', '代理商签名'): {},
            (self.classify, 'leshua_sign_name', '乐刷签名'): {},
            (self.classify, 'is_open', '是否启用'): {
                'default': 'no', 'type': 'select', 'option': ['yes', 'no']
            },
            ...
            (self.classify, verify_code_sms.label, '代理商验证码模板id'): {},
